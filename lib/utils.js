...
export function getPostExpiryDate(post) {
  if (!post) return null;

  const explicitExpiry = toDateValue(post.expiresAt);
  if (explicitExpiry) return explicitExpiry;

  const deadline = toDateValue(post.deadline);
...
export function isPostExpired(post) {
  const expiry = getPostExpiryDate(post);
