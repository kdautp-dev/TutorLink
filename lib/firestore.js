...
export async function claimPost(postId, tutorProfile) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const helperName = tutorProfile?.name || "Helper";
    const helperEmail = tutorProfile?.email || "";

    transaction.update(postRef, {
      status: post.helperId ? post.status : POST_STATUSES.IN_PROGRESS,
      helperId: post.helperId || tutorProfile.uid,
      helperName: post.helperName || helperName,
      helperEmail: post.helperEmail || helperEmail,
