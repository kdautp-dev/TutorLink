...
export async function createUserProfile(uid, profile) {
  ...
  await setDoc(userRef, {
    ...
    subjectsHelping: profile.subjectsHelping || [],
    subjectsRequesting: profile.subjectsRequesting || [],
    bio: profile.bio || "",
...
export async function createPost(data) {
  requireDb();
  const deadlineValue = data.deadline ? new Date(data.deadline) : null;
  const expiresAt = deadlineValue || new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
